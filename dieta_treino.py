...
if projeto:
    ...
    if aba == "Treinos" and not df.empty and "Data" in df.columns:
        df["Mês"] = pd.to_datetime(df["Data"], errors='coerce', dayfirst=True).dt.strftime("%B/%Y")
    ...
    if aba == "Treinos":
        ...
        with st.form("form_editar_treino"):
            ...
            if submitted:
                ...
                if editar and data_treino.strftime('%d/%m/%Y') in df["Data"].values:
                    df.loc[df["Data"] == data_treino.strftime('%d/%m/%Y'), "Treino"] = treino_desc
    ...
    if aba == "Dieta":
        ...
        with st.form("form_editar_dieta"):
            ...
            if salvar_alimento:
                nova_linha = {
                    "Data": data_refeicao.strftime('%d/%m/%Y'),
                    "Refeição": refeicao,
                    "Alimentos": alimento,
                    "Unidade": unidade,
